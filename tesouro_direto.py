...
parser.add_argument(
    ...
    "--title",
    type=str,
    choices=TREASURE_LIST,
    help="Uma string representando o código do título",
)
parser.add_argument(
    "-i",
    "--info",
    action="store_true",
    help="Informações sobre o código de cada tesouro",
)
parser.add_argument("-y", "--year", type=int, help="Um inteiro representando o ano")
parser.add_argument(
    "-d",
    "--download",
    action="store_true",
    help="Adicione para ser feito o download do gráfico",
)
...
def get_treasure_history_data(title, year, download):
    ...
    df.rename(columns={"Vencimento": "DATA", "Unnamed: 5": "PU_BASE"}, inplace=True)
...
def get_treasure_data():
    ...
    field_names = [
        th.text.strip() for th in table.find("tr", class_="tabelaTitulo").find_all("th")
    ]
    pretty_table.field_names = field_names

    treasure_title = [
        tr.find("td").text.strip()
        for tr in table.find_all("tr", class_="tituloprefixado")
    ]
